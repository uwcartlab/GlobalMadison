...
function initialize(){
    ...
    //data loading
    Promise.all([
        d3.json("data/capitol-square/routesv2.geojson"),
        d3.json("data/capitol-square/PointsofInterestv2.geojson"),
        d3.json("data/capitol-square/pointField.geojson"),
...
/*UNIVERSAL MODAL FUNCTIONS*/
    ...
    function addModalTitle(type){
        let title = pois.features[currentSite].properties.title + " - Landmark " + (pois.features[currentSite].properties.id+1) + " of 2";
...
/*MOVE MAP*/
    ...
    function moveMap(){
        if (siteCoords.length > 1 && currentSite > 0){
            ...
            var bounds = L.latLngBounds([43.0756713,-89.383092], [siteCoords[currentSite].lat, siteCoords[currentSite].lng]);
        };
...
function triggerIconBubble(){
    if ($('#iconClickBubble span').html().length < 1){
        ...
        $('#iconClickBubble span').html("When you arrive here, tap icon<br/>for landmark information");
      
        //find the right icon
        $(".leaflet-marker-icon").each(function(){
            console.log(this)
            var icon = screenSize == 'small' ? "img/icons/street40_red.png" : "img/icons/street24_red.png";
